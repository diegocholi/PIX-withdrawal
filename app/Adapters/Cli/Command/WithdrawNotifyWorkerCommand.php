...
final class WithdrawNotifyWorkerCommand extends HyperfCommand
{
    protected ?string $signature = 'withdraw:worker:notify'
    ...
    protected string $description = 'Inicia o worker Kafka responsavel pelo envio de notificacoes de saque por email.';

    public function __construct(
        private readonly WithdrawNotificationWorker $withdrawNotificationWorker,
    ...
    )
    {
        ...
        $this->setAliases(['worker:notify:run']);
    }
    ...
    public function handle(): int
    {
        ...
        try {
            $result = $this->withdrawNotificationWorker->run($runtimeOptions);
        ...
        $this->line(json_encode([
            'command' => 'withdraw:worker:notify',
    ...
    private function resolveRuntimeOptions(): ?WorkerRuntimeOptions
    {
        ...
        $groupId = $groupId === '' ? $this->kafkaConfig->consumerGroup('notification') : $groupId;

        $topic = trim((string) $this->option(CliOptionName::TOPIC));
        $topic = $topic === '' ? $this->kafkaConfig->notificationEmailWithdrawTopic() : $topic;
